...
const onNuxtReady = (callback: () => any) => {
...
export interface _AsyncData<DataT, ErrorT> {
  ...
  clear: () => void;
  resolve: () => Promise<void> | undefined;
  error: Ref<ErrorT | null>;
  status: Ref<AsyncDataRequestStatus>;
}

export type AsyncData<Data, Default, Error> = (
  | {
      status: "success";
      data: Data;
      pending: false;
  ...
  refresh: (opts?: AsyncDataExecuteOptions) => Promise<Data>;
  execute: (opts?: AsyncDataExecuteOptions) => Promise<Data>;
  clear: () => void;
  resolve: () => Promise<void> | undefined;
};

// TODO: remove boolean option in Nuxt 4
const isDefer = (dedupe?: boolean | "cancel" | "defer") =>
  dedupe === "defer" || dedupe === false;
...
>(
  ...
  const autoKey =
    typeof args[args.length - 1] === "string" ? args.pop() : undefined;
  if (typeof args[0] !== "string") {
    args.unshift(autoKey);
  }

  // eslint-disable-next-line prefer-const
  let [key, _handler, options = {}] = args as [
    string,
    (ctx?: NuxtApp) => Promise<ResT>,
  ...
  const handler =
    ...
    !nuxtApp.ssrContext?._sharedPrerenderCache
      ? _handler
      : () => {
          const value = nuxtApp.ssrContext!._sharedPrerenderCache!.get(key);
          if (value) {
            return value as Promise<ResT>;
          }
  ...
  if (import.meta.dev && typeof options.dedupe === "boolean") {
    console.warn(
      "[nuxt] `boolean` values are deprecated for the `dedupe` option of `useAsyncData` and will be removed in the future. Use 'cancel' or 'defer' instead."
    );
  }
  ...
  const asyncData = { ...nuxtApp._asyncData[key] } as _AsyncData<
    DataT | DefaultT,
  ...
  asyncData.resolve = async () => nuxtApp._asyncDataPromises[key];
  ...
  asyncData.refresh = asyncData.execute = (opts = {}) => {
    ...
    const promise = new Promise<ResT>((resolve, reject) => {
      ...
      .catch((error: any) => {
        ...
        asyncData.error.value = createError<NuxtErrorDataT>(
          error
        ) as NuxtErrorDataT extends Error | NuxtError
          ? NuxtErrorDataT
          : NuxtError<NuxtErrorDataT>;
  ...
  const fetchOnServer =
    options.server !== false && nuxtApp.payload.serverRendered;
  ...
  if (import.meta.client) {
    ...
    if (
      ...
      console.warn(
        ...
        }] Component is already mounted, please use $fetch instead. See https://nuxt.com/docs/getting-started/data-fetching`
      );
    }
    ...
    if (
      fetchOnServer &&
      nuxtApp.isHydrating &&
      (asyncData.error.value || hasCachedData())
    ) {
  ...
  // Allow awaiting on asyncData with resolve function
  return reactive({
    ...asyncData,
...
export function useLazyAsyncData<
  ...
  PickKeys extends KeysOf<DataT> = KeysOf<DataT>,
  DefaultT = null
>(...args: any[]): AsyncData<PickFrom<DataT, PickKeys>, DefaultT, DataE> {
  const autoKey =
    typeof args[args.length - 1] === "string" ? args.pop() : undefined;
  if (typeof args[0] !== "string") {
    args.unshift(autoKey);
  }
  const [key, handler, options = {}] = args as [
    string,
    (ctx?: NuxtApp) => Promise<ResT>,
    AsyncDataOptions<ResT, DataT, PickKeys, DefaultT>
  ];
