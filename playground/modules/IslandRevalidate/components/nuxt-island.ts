...
import { getFragmentHTML } from "../utils/getFragmentHtml";

import {
  appBaseURL,
  remoteComponentIslands,
  selectiveClient,
...
const components = import.meta.client
    ...
    if (!components!.has(component)) {
      promises.push(
        (async () => {
          ...
          const c = await import(/* @vite-ignore */ chunkSource).then(
            (m) => m.default || m
          );
...
export default defineComponent({
  ...
  async setup(props, { slots, expose, emit }) {
    ...
    const key = ref(0);
    const canLoadClientComponent = computed(
      () =>
        selectiveClient &&
        (props.dangerouslyLoadClientComponents || !props.source)
    );
      ...
      props.props
        ? Object.fromEntries(
            Object.entries(props.props).filter(
              ([key]) => !key.startsWith("data-v-")
            )
          )
        : {}
    );
    const hashId = computed(() =>
      hash([props.name, filteredProps.value, props.context, props.source])
    );
    const instance = getCurrentInstance()!;
    const event = useRequestEvent();

    // TODO: remove use of `$fetch.raw` when nitro 503 issues on windows dev server are resolved
    const eventFetch = import.meta.server
      ? event!.fetch
      : import.meta.dev
      ? $fetch.raw
      : globalThis.fetch;
    ...
    if (instance.vnode.el) {
      const slots = toRaw(
        nuxtApp.payload.data[`${props.name}_${hashId.value}`]
      )?.slots;
      if (slots) {
        payloads.slots = slots;
      }
      if (selectiveClient) {
        const components = toRaw(
          nuxtApp.payload.data[`${props.name}_${hashId.value}`]
        )?.components;
        if (components) {
    ...
    const availableSlots = computed(() =>
      [...ssrHTML.value.matchAll(SLOTNAME_RE)].map((m) => m[1])
    );
    const html = computed(() => {
      ...
      if (import.meta.client && !canLoadClientComponent.value) {
        for (const [key, value] of Object.entries(payloads.components || {})) {
          html = html.replace(
            new RegExp(
              ` data-island-uid="${uid.value}" data-island-component="${key}"[^>]*>`
            ),
            (full) => {
    ...
    const cHead = ref<Record<"link" | "style", Array<Record<string, string>>>>({
      link: [],
      style: [],
    });
    useHead(cHead);
    ...
    async function _fetchComponent(force = false) {
      ...
      const url =
        remoteComponentIslands && props.source
          ? new URL(`/__nuxt_island/${key}.json`, props.source).href
          : `/__nuxt_island/${key}.json`;
      ...
      const r = await eventFetch(
        withQuery(
          (import.meta.dev && import.meta.client) || props.source
            ? url
            : joinURL(config.app.baseURL ?? "", url),
          {
            ...props.context,
            props: props.props ? JSON.stringify(props.props) : undefined,
          }
      ...
      const result =
        import.meta.server || !import.meta.dev
          ? await r.json()
          : (r as FetchResponse<NuxtIslandResponse>)._data;
    ...
    async function updateComponent(res: NuxtIslandResponse) {
      try {
        cHead.value.link = res.head.link;
        cHead.value.style = res.head.style;
        ssrHTML.value = res.html.replaceAll(
          DATA_ISLAND_UID_RE,
          `data-island-uid="${uid.value}"`
        );
        key.value++;
        error.value = null;
    ...
    async function fetchComponent(force = false) {
      ...
      await updateComponent(await nuxtApp[pKey][uid.value]);
    }
    ...
    if (import.meta.client) {
      nuxtApp.payload.data.revalidateIsland ||= {};
      nuxtApp.payload.data.revalidateIsland[uid.value] = () => ({
        key: `${props.name}_${hashId.value}`,
      ...
      watch(revalidate, async () => {
        const key = `${props.name}_${hashId.value}`;
        // console.log("watcher:key", key);
        const v = nuxtApp.payload.data.revalidateIsland[key]();
        if (v) updateComponent(v);
        // console.log("watcher:island", v);
      });
    }
      ...
      return [
        ...
        withMemo(
          ...
          () => {
            ...
            const isKeyOdd =
              teleportKey.value === 0 ||
              !!(teleportKey.value && !(teleportKey.value % 2));

            if (
              uid.value &&
              html.value &&
              (import.meta.server || props.lazy
                ? canTeleport
                : mounted.value || instance.vnode?.el)
            ) {
                    ...
                    createVNode(
                      ...
                      // use different selectors for even and odd teleportKey to force trigger the teleport
                      {
                        to: import.meta.client
                          ? `${isKeyOdd ? "div" : ""}[data-island-uid="${
                              uid.value
                            }"][data-island-slot="${slot}"]`
                          : `uid=${uid.value};slot=${slot}`,
                      },
                      {
                        default: () =>
                          (payloads.slots?.[slot].props?.length
                            ? payloads.slots[slot].props
                            : [{}]
                          ).map((data: any) => slots[slot]?.(data)),
                      }
                    )
                  );
                }
                ...
                if (import.meta.server) {
                    ...
                    for (const [id, info] of Object.entries(
                      ...
                      for (const slot in slots) {
                        replaced = replaced.replaceAll(
                          `data-island-slot="${slot}">`,
                          (full) => full + slots[slot]
                        );
                      }
                      teleports.push(
                        createVNode(
                          Teleport,
                          { to: `uid=${uid.value};client=${id}` },
                          {
                  ...
                  for (const [id, info] of Object.entries(
                    ...
                    const vnode = createVNode(
                      Teleport,
                      {
                        to: `${isKeyOdd ? "div" : ""}[data-island-uid='${
                          uid.value
                        }'][data-island-component="${id}"]`,
                      },
                      {
                        default: () => {
                          return [
                            h(
                              ...
                              Object.fromEntries(
                                Object.entries(slots || {}).map(([k, v]) => [
                                  ...
                                  () =>
                                    createStaticVNode(
                                      `<div style="display: contents" data-island-uid data-island-slot="${k}">${v}</div>`,
                                      1
                                    ),
                                ])
                              )
