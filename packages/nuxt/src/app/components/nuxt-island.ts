...
export default defineComponent({
  ...
  async setup(props, { slots, expose }) {
    ...
    if (nuxtApp.isHydrating) {
      payloads.slots =
        toRaw(nuxtApp.payload.data[`${props.name}_${hashId.value}`])?.slots ??
        {};
      payloads.components =
        toRaw(nuxtApp.payload.data[`${props.name}_${hashId.value}`])
          ?.components ?? {};
    }
    ...
    if (import.meta.client && nuxtApp.isHydrating) {
      ssrHTML.value =
        getFragmentHTML(instance.vnode?.el ?? null, true)?.join("") || "";
    }
    ...
    async function _fetchComponent(force = false) {
      ...
      if (nuxtApp.payload.data[key]?.html && !force) {
          ...
          () => {
            ...
            if (
              ...
              (import.meta.server || props.lazy
                ? canTeleport
                : mounted.value || nuxtApp.isHydrating)
            ) {
